...
# Use "drop_na" for listwise NA removal
# There are a lot of ways to deal with NAs explained in the last Data Preprocessing workshop!!
colnames(data)
numeric_vars <- dplyr::select(data, "call", "race","n_jobs","years_exp", "frac_black","frac_white", 
                              ...
                              "frac_colp_emp_zip","l_inc_emp_zip") %>% 
...
# must convert data to dataframe or matrix for cor matrix
attributes(numeric_vars)
...
# Plot distribution of binary response/DV variable
par(mfrow=c(1,1))
barplot(table(numeric_vars$call), beside = TRUE)
barplot(table(numeric_vars$race, numeric_vars$call), beside = TRUE)
