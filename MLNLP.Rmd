...
mmALL_073120_csv <- readr::read_csv("mmALL_073120_csv.csv") %>%
  filter(year=="2020")
...
library(tidytext)
?tidytext::unnest_tokens  # Help R Doc for needed function
colnames(mmALL_073120_csv) # copy proper column name

# remove trailing and leading spaces 
mmALL_073120_csv$notes <- stringr::str_trim(mmALL_073120_csv$notes, side = "both")
...
stem_counts_noid <- mmALL_073120_csv %>%
  unnest_tokens(word, notes) %>%
  anti_join(stop_words) %>%
   mutate(stem = wordStem(word)) %>%
  count(stem) %>%
    arrange(desc(n))
View(stem_counts_noid)
...
top_tfidf %>%
  ...
  filter(region=="Europe" |country =="Russia" ) %>%
...
Text2vec Package - handle large text datasets with high efficiency 
Topicmodels Package - LDA, CTM, model evaluation, viz 
MALLET Package - LDA, Hierarchical LDA 
TM Package LSAfun Package - Latent Semantic Analysis (LSA) 
tsne Package - dimensionality reduction and data visualization
...
mm_lda
topicdata <- as.data.frame(terms(mm_lda, 10))
View(topicdata) 
...
View(get_sentiments(lexicon = "nrc"))
